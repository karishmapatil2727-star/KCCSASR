...
function bindCloseTrailBalanceSheetstoGrid(fromDate, toDate, forInstId) {
    if ($.fn.DataTable.isDataTable("#tblOpentrailBalanceSheetReport")) {
        $('#tblOpentrailBalanceSheetReport').DataTable().draw();
        $('#tblOpentrailBalanceSheetReport').DataTable().destroy();
        $('#tblOpentrailBalanceSheetReport tbody').empty();
    }
    $('#tblOpentrailBalanceSheetReport').DataTable({
        bProcessing: true,
        pageLength: 100,
        dom:
        ...
        buttons: [
            ...
            },
            {
                extend: 'pdfHtml5',
                pageSize: 'A4',
                filename: 'Open Trail Balance Group Wise_' + now.getDate() + '-' + (now.getMonth() + 1) + '-' + now.getFullYear(),
                title: 'Export',
                header: true,
                customize: function (doc) {
                    doc.content.splice(0, 1, {
                        text: [{
                            ...
                            text: 'Open Trail Balance (Group Wise) From(' + fromDate.getDate() + '/' + (fromDate.getMonth() + 1) + '/' + fromDate.getFullYear() + '-' + toDate.getDate() + '/' + (toDate.getMonth() + 1) + '/' + toDate.getFullYear() + ')',
                            bold: true,
                            fontSize: 11
                        }],
                        margin: [0, 0, 0, 12],
            ...
            },
            {
                ...
                message: "<div class='row'><h3 class='text-center'>Khalsa College Charitable Society, Amritsar</h3></div>" +
                    ...
                    "<div class='row exportoption'><h4 class='text-center'> Open Trail Balance (Group Wise) From (" + fromDate.getDate() + "/" + (fromDate.getMonth() + 1) + "/" + fromDate.getFullYear() + " -" + toDate.getDate() + "/" + (toDate.getMonth() + 1) + "/" + toDate.getFullYear() + ")</h4></div> ",
        ...
        ajax: {
            url: "OpenTrailBalanceJsonReport",
                ...
                render: function (data, type, row) {
                    if (row.AccountGroupName != null) {
                        ...
                        if (row.IsLedger == true) {
                            link.attr("onclick", "bindLedgerSummaryRepory('" + fromDate + "','" + fromDate + "','" + row.AccountGroupId + "','" + instituteId + "','" + row.AccountGroupName + "')");
                        }
                        else {
                            link.attr("onclick", "bindSchGroupSummary('" + row.AccountGroupId + "','" + row.AccountGroupName + "','" + fromDate + "','" + instituteId + "','" + fromDate + "')");
                        }
                        link.text(row.AccountGroupName);
                        var html = link[0].outerHTML;
                        return html;
