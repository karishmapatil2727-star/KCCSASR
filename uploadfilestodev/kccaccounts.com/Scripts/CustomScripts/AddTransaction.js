...
function AddTransactiontoDb(narration) {
    ...
    if (trasDate.length == 3) {
        ...
        // var tzoffset = (new Date(trasDate[2], month, trasDate[0])).getTimezoneOffset() * 60000; //offset in milliseconds
        // var trnsactionDate = (new Date(Date.now() - tzoffset));
        // if (dept == '300010') {
            var get_transaction_date = new Date(trasDate[2], month, trasDate[0]);
            trnsactionDate = get_transaction_date.getFullYear()+'-' + (get_transaction_date.getMonth()+1) + '-'+get_transaction_date.getDate() + ' 00:00:00.000';
            // console.log(get_transaction_date);
            // console.log(trnsactionDate);
            // return false;
        // }
        ...
        // if (dept == '300010') {
            var transactionsViewModel = { DeptId: dept, VocherType: selectedVocher, NewVocherNo: vocherNo, ChequeorCash: chequeOrCash, TotalDebit: totalDebit, TotalCredit: totalCredit, MasterNarration: narration, accountLedgers: ledgerData, TransactionDate: trnsactionDate,FinancialYearId: userData.FinancialYearId };
        //     console.log(transactionsViewModel);
        //     return false;
        // }
        // var transactionsViewModel = { DeptId: dept, VocherType: selectedVocher, NewVocherNo: vocherNo, ChequeorCash: chequeOrCash, TotalDebit: totalDebit, TotalCredit: totalCredit, MasterNarration: narration, accountLedgers: ledgerData, TransactionDate: trnsactionDate.toISOString(),FinancialYearId: userData.FinancialYearId };
...
function getCurrentFiscalYear() {
    ...
    var newFinYearId = 13;
    var prevFinYearId = newFinYearId-1;
    
    var month = todayDate.getMonth() + 1;
    var dateT = todayDate.getDate() + "." + month + "." + todayDate.getFullYear();

    if ( parseInt(userData.FinancialYearId) == prevFinYearId ) {
        var month = selectedFinYear.getMonth() + 1;
        if (selectedFinYear.getDay() == 0) { // if sunday then allow entry for saturday
       ...
       // $('#btnProceed').hide();
        return true;
    } else if(parseInt(userData.FinancialYearId) < prevFinYearId){
        $('#txtDate').val(dateT);    
        $('#btnProceed').hide();
        return true;
    }
