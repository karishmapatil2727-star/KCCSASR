...
function bindCloseTrailBalanceSheetstoGrid(fromDate, toDate, forInstId) {
    if ($.fn.DataTable.isDataTable("#tblOpenTrailBalanceDetailedSheetReport")) {
        $('#tblOpenTrailBalanceDetailedSheetReport').DataTable().draw();
        $('#tblOpenTrailBalanceDetailedSheetReport').DataTable().destroy();
        $('#tblOpenTrailBalanceDetailedSheetReport tbody').empty();
    }
    $('#tblOpenTrailBalanceDetailedSheetReport').DataTable({
        bProcessing: true,
        pageLength: 100,
        dom:
        ...
        buttons: [
            ...
            },
            {
                extend: 'pdfHtml5',
                pageSize: 'A4',
                filename: 'Open Trail Balance Detailed_' + now.getDate() + '-' + (now.getMonth() + 1) + '-' + now.getFullYear(),
                title: 'Export',
                header: true,
                customize: function (doc) {
                    doc.content.splice(0, 1, {
                        text: [{
                            ...
                            text: 'Open Trail Balance Detailed From(' + fromDate.getDate() + '/' + (fromDate.getMonth() + 1) + '/' + fromDate.getFullYear() + '-' + toDate.getDate() + '/' + (toDate.getMonth() + 1) + '/' + toDate.getFullYear() + ')',
                            bold: true,
                            fontSize: 11
                        }],
                        margin: [0, 0, 0, 12],
            ...
            },
            {
                ...
                message: "<div class='row'><h3 class='text-center'>Khalsa College Charitable Society, Amritsar</h3></div>" +
                    ...
                    "<div class='row exportoption'><h4 class='text-center'> Open Trail Balance Detailed From (" + fromDate.getDate() + "/" + (fromDate.getMonth() + 1) + "/" + fromDate.getFullYear() + " -" + toDate.getDate() + "/" + (toDate.getMonth() + 1) + "/" + toDate.getFullYear() + ")</h4></div> ",
        ...
        ajax: {
            url: "OpenTrailBalanceDetailedJsonReport",
...
function bindTransactionDetails(accountGroupId, accountGroupName, fromDate, toDate, className, instituteId) {
    ...
    bindLedgerSummaryRepory(fromDate, fromDate, accountGroupId, instituteId, accountGroupName);
}
